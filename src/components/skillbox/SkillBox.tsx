...
const SkillBox = (props: skillprops) => {
    ...
    for (let index = 0; index < Math.trunc(props.skillRating/2); index++) {
        rating_star.push(<img src={process.env.PUBLIC_URL+"/assets/images/full_star.png"} alt="rating" />);        
    }
    if(!Number.isInteger(rating_calculate)) {
        rating_star.push(<img src={process.env.PUBLIC_URL+"/assets/images/half_star.png"} alt="rating" />);
    }
    for (let index = Math.ceil(rating_calculate); index < 5; index++) {
        rating_star.push(<img src={process.env.PUBLIC_URL+"/assets/images/star.png"} alt="rating" />);        
    }
    return (
        <div key={props.uniqueKey} className={styles.skills_div}>
            ...
            <img 
                src={process.env.PUBLIC_URL+"/assets/images/skills/"+props.skillIconName}
                onError={({ currentTarget }) => {
                    currentTarget.onerror = null;
                    currentTarget.src = process.env.PUBLIC_URL+"/assets/images/error_image.png"
                }} 
                alt="skill-logo" />
